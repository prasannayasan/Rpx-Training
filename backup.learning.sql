...
(SELECT rpad(colunm,MAX(LENGTH(colunm)) OVER())||chr(9)|| '|' || chr(32) 
	||  rpad(type, MAX(LENGTH(type)) OVER()) || chr(9) || '|' || chr(32) 
	|| 	rpad(collations, 9) || '|' || chr(32)
	|| 	rpad(nullable, 9) || '|' || chr(32) 
...
SELECT rpad(colm, len) || chr(9) || 'xavier' || chr(9) || 123 FROM 
( SELECT colm, MAX (LENGTH(colm)) OVER () AS len FROM 
	( SELECT UNNEST ( ARRAY [ 'name', 'updated_at', 'long long long long' ] ) AS colm )T )T;
...
CREATE OR REPLACE FUNCTION get_function_oid(functionname text)
RETURNS oid AS $$
DECLARE
output oid;
BEGIN
SELECT functionname::pg_catalog.regproc::pg_catalog.oid into output;
return output;
end; $$
language plpgsql;


CREATE OR REPLACE FUNCTION get_function_description(functionname text)
RETURNS text AS $$
DECLARE
output text;
BEGIN
SELECT pg_catalog.pg_get_functiondef(get_function_oid(functionname)) into output;
return output;
end; $$
language plpgsql;

CREATE OR REPLACE FUNCTION get_view_description(viewname text)
RETURNS text AS $$
DECLARE
output text;
BEGIN
SELECT pg_catalog.pg_get_viewdef(c.oid, true) into output
FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON c.relnamespace = n.oid WHERE c.oid = get_oid(viewname);
return output;
end; $$
language plpgsql;
...
get_view_descrip(functionname text)
get_function_oid(functionname text)
get_func_descrip(functionname text)
